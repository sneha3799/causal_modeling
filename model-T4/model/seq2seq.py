...
class AttentionDecoder(nn.Module):
    def __init__(self, output_dim, x_static_size, emb_dim, hid_dim, n_layers, dropout, treatment_dim=1):
        super().__init__()

        self.hid_dim = hid_dim
        self.output_dim = output_dim
        self.n_layers = n_layers
        self.treatment_dim = treatment_dim  # Number of treatment dimensions (e.g., 2 for dosage and timing)
        ...
        # Modified to accept treatment dimensions (dosage and timing)
        self.rnn = nn.LSTM(emb_dim*2+treatment_dim, hid_dim, n_layers, dropout=dropout, batch_first=True)

        # Original binary treatment branches
        ...
        # Additional output branches for handling continuous insulin dosage
        self.fc_out_dose = torch.nn.Sequential(
            nn.Linear(hid_dim * 2, hid_dim),
            torch.nn.ReLU(),
            torch.nn.Linear(hid_dim, 1))
            
        # Output branch for timing effect
        self.fc_out_timing = torch.nn.Sequential(
            nn.Linear(hid_dim * 2, hid_dim),
            torch.nn.ReLU(),
            torch.nn.Linear(hid_dim, 1))
    ...
    def forward(self, input, x_static, treatment_cur, treatment_next, hidden, cell, encoder_outputs, mask):
        """
        Forward pass with support for non-binary treatments
        
        Args:
            input: Input value (previous glucose value)
            x_static: Static patient features
            treatment_cur: Current treatment (can be multi-dimensional)
            treatment_next: Next treatment (can be multi-dimensional)
            hidden, cell: Hidden states from previous step
            encoder_outputs: Outputs from encoder
            mask: Mask for valid timesteps
            
        Returns:
            predictions: Tuple of (factual, counterfactual) predictions
            hidden_states: Tuple of (hidden, cell) states
            output: Raw output
            ps_output: Propensity score output
        ...
        # Handle multi-dimensional treatments
        if not isinstance(treatment_cur, torch.Tensor) or len(treatment_cur.shape) == 1:
        ...
        # Handle different treatment modalities
        if isinstance(treatment_next, tuple) or (isinstance(treatment_next, torch.Tensor) and treatment_next.shape[-1] > 1):
            # Multi-dimensional treatment (dosage and timing)
            if isinstance(treatment_next, tuple):
                dosage, timing = treatment_next
            else:
                dosage = treatment_next[:, 0].unsqueeze(-1)
                timing = treatment_next[:, 1].unsqueeze(-1)
                
            # Compute base prediction
            base_prediction = self.fc_out_0(output)
            
            # Apply dosage effect (continuous value from 0 to 1)
            dosage_effect = dosage * self.fc_out_1(output) + (1-dosage) * self.fc_out_0(output)
            
            # Apply timing effect
            timing_normalized = (timing + 30) / 60  # Convert -30 to +30 range to 0-1
            timing_effect = self.fc_out_timing(output) * timing_normalized
            
            # Combine effects
            prediction = dosage_effect + timing_effect
            
            # For counterfactual, we use opposite values
            cf_dosage = 1 - dosage
            cf_timing = -timing  # Reverse the timing
            cf_timing_normalized = (cf_timing + 30) / 60
            
            dosage_effect_cf = cf_dosage * self.fc_out_1(output) + (1-cf_dosage) * self.fc_out_0(output)
            timing_effect_cf = self.fc_out_timing(output) * cf_timing_normalized
            
            prediction_cf = dosage_effect_cf + timing_effect_cf
            
        else:
            # Original binary treatment approach
            if not isinstance(treatment_next, torch.Tensor) or len(treatment_next.shape) == 1:
